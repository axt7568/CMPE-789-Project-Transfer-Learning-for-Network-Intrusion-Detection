...
# make FTP-Patator feature
y_2 = np.ones(data_2.shape[0])
y_FTP_Patator = pd.DataFrame(y_2)

# make SSH-Patator feature
y_3 = np.full(data_3.shape[0], 2)
y_SSH_Patator = pd.DataFrame(y_3)

# make DoS GoldenEye feature
y_4 = np.ones(data_4.shape[0])
y_DoS_GoldenEye = pd.DataFrame(y_4)

# make DoS Hulk feature
y_5 = np.zeros(data_5.shape[0])
y_DoS_Hulk = pd.DataFrame(y_5)

# merging the original dataframe
X = pd.concat([data_1, data_2, data_3, data_4, data_5], sort=True)
y = pd.concat([y_benign, y_FTP_Patator, y_SSH_Patator, y_DoS_GoldenEye, y_DoS_Hulk], sort=True)
...
if (train_model == 1):
    ...
    # freeze_top_layers
    model.save("saved_models/ids2018c_tf_ids2017c_s2_fz_cxl.h5")
    ...
    # Load Trained Model
    print("\nLoading Trained Model")
    model = load_model("saved_models/ids2018c_tf_ids2017c_s2_fz_cxl.h5")
    model.summary()
